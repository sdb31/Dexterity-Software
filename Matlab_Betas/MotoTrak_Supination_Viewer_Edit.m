...
filename = [handles.file(1:end-10) '_TRIAL' ...
    num2str(handles.cur_trial,'%03.0f') '.pdf'];                            %Create a default filename for the PNG file.
[filename, path] = uiputfile('*.pdf','Name Image File',filename);           %Ask the user for a new filename.
...
print(gcf,[path, filename],'-dpdf','-r300');                                %Save the current image as a PDF file.
...
handles.fig = figure('MenuBar','none',...
    'numbertitle','off',...
    'name',['Dexterity Trial Viewer: ' handles.file],...
    'units','centimeters',...
    'resize','on',...
