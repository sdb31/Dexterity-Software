function Knob_Analysis_Viewer_V4(varargin)
valid = 0;
...
title([handles.cur_animal ': Mean Trial with 95% Confidence Interval'], ...
...
%This function toggles which axes to show
function Axes_Select(handles)

%Clear the data axes
cla(handles.pre.axes, 'reset');
cla(handles.post.axes, 'reset');
...
%See what variable we want to look at
if strcmpi(handles.analysis, 'data')
    data_axes = 'on';
    trial_axes = 'off';
    
    set(handles.session_list_pre, 'Visible', 'off');
    set(handles.session_label_pre, 'Visible', 'off');
    
    set(handles.session_list_post, 'Visible', 'off');
    set(handles.session_label_post, 'Visible', 'off');
    
    
elseif strcmpi(handles.analysis, 'trial')
    
    data_axes = 'off';
    trial_axes = 'on';
    
    set(handles.session_list_pre, 'Visible', 'on');
    set(handles.session_label_pre, 'Visible', 'on');
    
    set(handles.session_list_post, 'Visible', 'on');
    set(handles.session_label_post, 'Visible', 'on');
end

%Set visibility on the data axes
set(handles.pre.axes, 'Visible', data_axes);                                                        
set(handles.post.axes, 'Visible', data_axes);   

%Set visibility on the trial axes
set(handles.pre.trial_axes, 'Visible', trial_axes);
set(handles.post.trial_axes, 'Visible', trial_axes);

%Set visibility on all the sub axes
for i = 1:9
    set(handles.pre.sub_axes(i), 'Visible', trial_axes);
    set(handles.post.sub_axes(i), 'Visible', trial_axes);
end
