...
handles.fig = figure('numbertitle','off',...
    ...
    'menubar', 'none',...
    'Position',[pos(3)/2-w/2, pos(4)/2-h/2, .9*w, 1*h]);
%% Button Group 1
handles.bg = uibuttongroup('Position',[.05 .05 .9 .9],...
    'Title','Load and Analyze Data',...
    'Fontsize',12, 'Fontweight', 'bold');
% handles.text = uicontrol(handles.fig,'style','text',...
%     'units','normalized',...
%     'Position',[.15 .9 .7 .1],...
%     'string','Load and Analyze Data',...
%     'fontsize', 14, 'fontweight', 'bold');
handles.Standard = uicontrol(handles.bg,'style','pushbutton',...
    'units','normalized',...
    'Position',[.05,.675,.9,.25],...
    'string','Standard',...
    'fontsize',12,'BackgroundColor',[.75 .9 .75]);
handles.Custom = uicontrol(handles.bg,'style','pushbutton',...
    'units','normalized',...
    'Position',[.05,.375,.9,.25],...
    'string','Custom',...
    'fontsize',12,'BackgroundColor',[.75 .75 .9]);
handles.Previous = uicontrol(handles.bg,'style','pushbutton',...
    'units','normalized',...
    'Position',[.05,.075,.9,.25],...
    'string','Previous',...
    'fontsize',12,'BackgroundColor',[.9 .75 .75]);
%% Button Group 2
% handles.bg2 = uibuttongroup('Position',[.1 .05 .8 .2],...
%     'Title','Syncing Repository',...
%     'Fontsize',12, 'Fontweight', 'bold');
% Push = uicontrol('Parent',handles.bg2,'Style','pushbutton','string','Push',...
%     'HorizontalAlignment', 'left', 'units', 'normalized', 'Position', [.05 .1 .4 .7],...
%     'Fontsize', 10) ;
% Pull = uicontrol('Parent',handles.bg2,'Style','pushbutton','string','Pull',...
%     'HorizontalAlignment', 'left', 'units', 'normalized', 'Position', [.55 .1 .4 .7],...
%     'Fontsize', 10) ;
% Push = uicontrol('Parent',handles.fig,'Style','pushbutton','string','Push',...
%     'HorizontalAlignment', 'left', 'units', 'normalized', 'Position', [.25 .05 .2 .125],...
%     'Fontsize', 10, 'Fontweight', 'bold') ;
% Pull = uicontrol('Parent',handles.fig,'Style','pushbutton','string','Pull',...
%     'HorizontalAlignment', 'left', 'units', 'normalized', 'Position', [.55 .05 .2 .125],...
%     'Fontsize', 10, 'Fontweight', 'bold') ;
set(handles.Standard, 'callback', @Standard);
set(handles.Custom, 'callback', @Custom);
set(handles.Previous, 'callback', @Previous);
...
fig = figure('numbertitle','off','units','centimeters',...
    'name','Dexterity: Custom Analysis','menubar','none',...
...
Analysis_Type = uicontrol('Parent', fig, 'Style', 'listbox', 'HorizontalAlignment', 'left',...
    ...
    'units', 'normalized', 'Position', [.525 .25 .45 .6],'userdata', FileInfo, 'Fontsize', 12);
Lab_Name = uicontrol('Parent', fig, 'Style', 'listbox', 'HorizontalAlignment', 'left','string', LabNames,...
    ...
    'Fontsize', 12);
Lab_Functions = uicontrol('Parent', fig, 'Style', 'text', 'String', 'Custom Analyses', ...
