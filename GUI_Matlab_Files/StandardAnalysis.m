...
if ~isempty(varargin) == 1;
    ...
    if strcmp(devices,'Vermicelli') == 0;
        ...
        for d = 1:length(devices);
            fig = figure('numbertitle','off','units','centimeters',...
                ...
                'position',[pos(3)/2-w/2, pos(4)/2-h/2, w, h]);
            tgroup = uitabgroup('Parent', fig);
            tabs = {config.data.animal.name};
            data = config.data;
            for i = 1:length(tabs);
                tab(i) = uitab('Parent', tgroup, 'Title', sprintf('Animal %s', char(tabs{i})));
                tabs_info(i,:) = get(tab(i));
                laststage(i) = data.animal(i).date(end);
                numberofsessions(i) = size(data.animal(i).date,2);
                AnimalName(i) = uicontrol('Parent', tab(i), 'Style', 'text', 'String', sprintf('Animal Name: %s', char(tabs{i})), ...
                    'HorizontalAlignment', 'left', 'units', 'normalized', 'Position', [.05 .85 .4 .1],...
                    'Fontsize', 10, 'Fontweight', 'normal') ;
                LastDateRun(i) = uicontrol('Parent', tab(i), 'Style', 'text', 'String', sprintf('Last Date Run: %s', laststage{i}), ...
                    'HorizontalAlignment', 'left', 'units', 'normalized', 'Position', [.05 .65 .4 .1],...
                    'Fontsize', 10, 'Fontweight', 'normal') ;
                NumberOfSessions(i) = uicontrol('Parent', tab(i), 'Style', 'text', 'String', sprintf('Number of Sessions: %d', numberofsessions(i)), ...
                    'HorizontalAlignment', 'left', 'units', 'normalized', 'Position', [.05 .45 .4 .1],...
                    'Fontsize', 10, 'Fontweight', 'normal') ;
                DevicesRuns(d) = uicontrol('Parent', tab(i), 'Style', 'text', 'String', sprintf('Devices: %s', devices{d}), ...
                    ...
                    'Fontsize', 10, 'Fontweight', 'normal') ;
                General_Analysis(i) = uicontrol('Parent', tab(i),'style','pushbutton','string','Graph Subject','HorizontalAlignment', 'left',...
                    'units','normalized','position',[.7 .625 .25 .25],'fontsize',10, 'callback', {@GeneralAnalysis,devices(d)},'userdata',data.animal(i));
                View_All_Animals(i) = uicontrol('Parent', tab(i),'style','pushbutton','string','Graph All Subjects','HorizontalAlignment', 'left',...
                    'units','normalized','position',[.7 .3 .25 .25],'fontsize',10, 'callback', {@AllAnimals,devices(d),rats},'userdata',data);
            end
        end
        return
    end
...
files = file_miner(datapath,{'*.ArdyMotor','*.MotoTrak','*.xls','*.xlsx'});                                 %Find all LPS *.ArdyMotor or *.MotoTrak files in the LPS folders.
...
if isempty(files)                                                           %If no files were found...
    errordlg('No data files were found in the that directory!');   %Show an error dialog box.
end
Excel_Check = strfind(char(files(1)),'xls');
if ~isempty(Excel_Check) == 1;
    [NUM,TXT,RAW] = xlsread(char(files(1)),1);
    if strcmp(TXT(1,1),'Vermicelli') == 1;
        data = VermicelliRead(files);
        rats = {data.animal.name};
    end
end
if ~isempty(Excel_Check) == 0;
                ...
                switch choice
                    case 'Yes'
                        if ~isempty(Excel_Check) == 0;
                            ...
                            config.data = data; config.time = date;
                            c = clock; hour = num2str(c(4)); minute = num2str(c(5)); year = num2str(c(1));
                            month = num2str(c(2)); day = num2str(c(3));
                            SessionName = [month day year '_' hour minute '_Analysis'];
                            datapath = 'C:\KnobAnalysis\ConfigFiles\';
...
for d = 1:length(devices)
    ...
    elseif strcmp(devices,'Vermicelli') == 0;
        ...
        for i = 5                                                             %Step through the 3 timescales.
            pos = [i*sp2+i*(w-6*sp2)/6, h-sp1-ui_h, (w-6*sp2)/6, ui_h];         %Set the position for each pushbutton.
            obj(i) = uicontrol(fig,'style','pushbutton','string',str,...
                'units','centimeters','position',pos,'fontsize',fontsize);      %Create pushbuttons for selecting the timescale.
        end
        pos = [75*sp2, sp1, 2*(w-6*sp2)/8, .9*ui_h];
        obj(8) = uicontrol(fig,'style','pushbutton','string','Annotate',...
...
%         set(obj(6),'callback',{@TrainingDays,obj,data});
        ...
        set(fig,'userdata',data_all);                                            %Save the plot data to the figure's 'UserData' property.
        Plot_Timeline(obj(1),[],obj,[],data_all);                                        %Call the function to plot the session data in the figure.
%         Make_Plot(data,ax,str,data,obj)
        set(fig,'ResizeFcn',{@Resize,ax,obj}); 
    end
end

function AnnotateNow(hObject,~,obj,data_all)
% msgbox('Hello!');
devices = 0;
if obj(2) ~= 0;
...
if devices == 0;
    device_title = inputdlg('What is the name of the device?', 'Device Name', [1 50]);
    config.devices = device_title;
else
...
%         set(obj(7),'callback',{@GroupAnimals,obj,data});
        ...
        sp2 = 0.01*w;                                                           %Set the horizontal spacing between axes and uicontrols.
        %     pos = [7*sp2,3*sp1,w-8*sp2,h-ui_h-5*sp1];                               %Set the position of the axes.
        pos = [7*sp2,4*sp1,w-8*sp2,h-ui_h-7*sp1];
        ax = axes('units','centimeters','position',pos,'box','on',...
            'linewidth',2);                                                     %Create axes for showing the log events histogram.
        obj = zeros(1,8);                                                       %Create a matrix to hold timescale uicontrol handles.
        str = {'Targeted Attempts',...
...
%         set(obj(6),'callback',{@TrainingDays,obj,data});
        %         set(obj(7),'callback',{@GroupAnimals,obj,data});
        set(fig,'userdata',data);                                            %Save the plot data to the figure's 'UserData' property.
        Plot_Timeline(obj(1),[],obj,[],data);                                        %Call the function to plot the session data in the figure.
%         Make_Plot(data,ax,str,data,obj)
        set(fig,'ResizeFcn',{@Resize,ax,obj});      
    end
end
...
for r = 1:length(data)                                                      %Step through each rat in the data structure.
    if obj(2) ~=0;
        ...
        plotdata(r).rat = data(r).name;
        if strcmpi(str,'Targeted Attempts');                              %If we're plotting overall hit rate...
            y = data(r).mean_targeted;
        elseif strcmpi(str,'Non Targeted Attempts');
            y = data(r).mean_nontargeted;
        elseif strcmpi(str,'Ratio');
            y = data(r).ratio;
        end
        plotdata(r).y = y; 
        plotdata(r).x = 1:length(plotdata(r).y);
        plotdata(r).date = data(r).date;
    end    
...
if isempty(fid)                                                             %If no text file handle was passed to this function...
    ...
    if any(strcmpi('Yes',all_variables))
        for r = 1:length(plotdata)
            ...
            for q = 1:size(Field_Names,1)
                temp_meat{q} = data(r).(Field_Names{q});
                if size(temp_meat{q},2) > size(temp_meat{q},1)
                    temp_meat{q} = temp_meat{q}';
                end
                % %                 meat(:,q) = temp_meat{q};
            end
            if size(Field_Names,1) > 2;
                Field_Names = Field_Names';
            end
            xlswrite(filename, Field_Names, r, 'A1')
            for m = 1:size(temp_meat,2)
                xlswrite(filename, temp_meat{m},r,[alphabet(m) '2'])
            end
...
if any(strcmpi({'image','both'},output))                                    %If the user wants to save an image...
    ...
    set(obj(find(obj)),'visible','off');                                               %Make all of the other figures invisible.
    ...
    set(obj(find(obj)),'visible','on');                                                %Make all of the other figures visible again.
    i = strcmpi(get(obj(find(obj)),'fontweight'),'bold');                              %Find the pushbutton with the bold fontweight.
    if obj(2) == 0;
        Plot_Timeline(obj(1),[],obj,[],data);
    else
...
if any(strcmpi({'spreadsheet','both'},output))                              %If the user wants to save a spreadsheet...
    ...
    fid = 1;
    if obj(2) == 0;
        Plot_Timeline(obj(1),[],obj,fid,data);
    else
        Plot_Timeline(obj(i),[],obj,fid,data);                                        %Call the subfunction to plot the data by the appropriate timeline.
    end
...
if obj(2) ~= 0;
    ...
    for r = 1:length(plotdata)
        c = r;
        line(plotdata(r).x,plotdata(r).y,'color',colors(c,:),...
            'linewidth',linewidth,'userdata',1,'parent',ax);
        for i = 1:size(plotdata(r).y,2);
            l(i) = line(plotdata(r).x(i),plotdata(r).y(i),...
                'markeredgecolor',colors(c,:),'linestyle','none',...
                'markerfacecolor',colors(c,:),'marker','.',...
                'linewidth',linewidth,'markersize',markersize,'userdata',2,...
                'parent',ax);
        ...
        %         x = get(ax,'XLim');
        x(r,:) = [plotdata(r).x(1) plotdata(r).x(end)];
        XMAX(r) = max(max(plotdata(r).x(end)));
        x(r,:) = x(r,:) + [-1 1];
        xlim(ax,[min(min(x)) max(max(x))]);
        %         y = get(ax,'YLim');
        if min(plotdata(r).y) < 0;
            y(r,:) = [1.1*min(plotdata(r).y) 1.1*max(plotdata(r).y)];
        else
            y(r,:) = [0 1.1*max(plotdata(r).y)];
        end
        %         y = y + [y(1)*-.1 y(2)*.1];
        ylim(ax,[min(min(y)) max(max(y))]);
        %         temp = get(ax,'ytick');                                                     %Grab the y-axis ticks.
        %         for i = 1:length(temp)                                                      %Step through the y-axis ticks.
        %             temp(i) = line(xlim(ax),temp(i)*[1,1],'color',[0.75 0.75 0.75],...
        %                 'linestyle','--','linewidth',0.5*linewidth,'userdata',3,...
        ...
        if length(plotdata) > 1;
            set(ax,'xticklabel',1:max(max(XMAX)),'fontsize',fontsize,'fontweight','bold',...
                'linewidth',linewidth,'XTick',1:max(XMAX));
            xlabel(ax,'Sessions');
        else
            set(ax,'xticklabel',plotdata(r).date,'fontsize',fontsize,'fontweight','bold',...
                'linewidth',linewidth,'XTick',1:length(plotdata(r).date));
        end
        ylabel(ax,str,'fontweight','bold','fontsize',1.1*fontsize);       
        %         box off;
    end
end
